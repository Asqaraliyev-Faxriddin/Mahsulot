...
export class VerificationService {
    ...
    private async throwIFUserExist(phone:string){
        const user = await this.prismaService.users.findUnique({where:{phone}})
    ...
    async sendOtp(payload:SendOtpDto){
        ...
        await this.redis.set(key,JSON.stringify(otp),SectoMills(400))
    ...
    async verifyOtp(payload:VerifyOtpDto){
        ...
        await this.redis.set(this.getKey(type,phone,true),JSON.stringify(otp),SectoMills(4000))

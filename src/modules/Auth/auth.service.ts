...
export class AuthService {
    ...
    async register(payload:Required<RegisterDto>){
        ...
        await this.verificationService.checkConfirmOtp({phone,type:EverificationTypes.REGISTER,otp})
        
        let hashpassword= await bcrypt.hash(password,10)
        

        let data = await this.prisma.users.create({data:{
            fullName,
            phone,
   ...
   async login(payload:LoginDto){
     ...
     let olduser =await this.PhoneAndPasswordCheck(payload.password,payload.phone)
    ...
    async RefresholdAcces(token:RefreshTokenDto){
        try {
           ...
           console.log("token",oldId);
            ...
            let checkUser = await this.prisma.users.findFirst({where:{id:oldId.id}})
    ...
    async reset_password(payload:Required<Reset_Password>){
        ...
        await this.verificationService.checkConfirmOtp({phone,type:EverificationTypes.RESET_PASSWORD,otp})
        ...
        let data = await this.prisma.users.update({where:{phone},data:{
